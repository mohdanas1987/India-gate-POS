...
**Gap:** still no category sidebar/filtering, no barcode-scanner wedge input handling, no weighted-product quantity entry, no split payments, and completed sales don't yet call the working hardware IPC (Phase 10) to actually print a receipt or open the cash drawer — the wiring exists, nothing calls it from a completed sale yet. This is a working, honestly-scoped slice of the plan's §49 design, not the finished screen.
